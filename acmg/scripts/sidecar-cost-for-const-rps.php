...
//benchmark.php
function testTp($rps) {
    $wrkClient = "wrk-684f4544cd-dhmxt";
    $nginxClient = "nginx-7d975b8d4b-pcn2m";
    $sidecarOut = "{$wrkClient} -c istio-proxy";
    $sidecarIn = "{$nginxClient} -c istio-proxy";

    $wrkCmd = "kubectl exec -it {$wrkClient} -- wrk -t16 -c100 -d40 -R{$rps} --latency http://nginx:80";
    $topStr = "top -b -n 60 -d 0.5";

    $wrkPath = "../wrk-result-2/sidecar-cost-result/wrk";
    if(!is_dir($wrkPath)) {
        mkdir($wrkPath, 0777, true);
    }
    $topPath = "../wrk-result-2/sidecar-cost-result/top";
    if(!is_dir($topPath)) {
        mkdir($topPath, 0777, true);
    }
    foreach (range(0, 15) as $repeat_index) {
        foreach (range(0, 3) as $thread_id) {
            $pid = pcntl_fork();
            if ($pid == -1) {
                echo "failed to fork!\n";
                exit;
                ...
                if ($thread_id == 0) { // start the wrk test process
                    ...
                    if ($thread_id == 1) {
                        $logPath = "{$topPath}/sidecar-out";
                        if(!is_dir($logPath)) {
                            mkdir($logPath, 0777, true);
                        }
                        $logFile = "{$logPath}/top_R{$rps}_{$repeat_index}";
                        $topCmd = "kubectl exec -it {$sidecarOut} -- {$topStr} | grep envoy > {$logFile}";
                    }
                    elseif ($thread_id == 2) {
                        $logPath = "{$topPath}/sidecar-in";
                        if(!is_dir($logPath)) {
                            mkdir($logPath, 0777, true);
                        }
                        $logFile = "{$logPath}/top_R{$rps}_{$repeat_index}";
                        $topCmd = "kubectl exec -it {$sidecarIn} -- {$topStr} | grep envoy > {$logFile}";
